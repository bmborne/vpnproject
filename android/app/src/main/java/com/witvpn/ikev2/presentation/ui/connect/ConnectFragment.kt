...
@AndroidEntryPoint
class ConnectFragment : BaseFragment<FragmentConnectBinding>(R.layout.fragment_connect), MainTabFragment.OnTabChanged {
    ...
    override fun initView() {
        ...
        val serverDraft = Server.getDraft()
        updateServerButton(serverDraft)
        initChart()
    }
    ...
    override fun initObserve() {
        shareViewModel.serverLiveData.observe(viewLifecycleOwner) {
            if (it.status == Status.SUCCESS) {
                updateServerButton(value = it.data, forceSave = true)
            }
        }
        ...
        viewModel.stateLiveData.observe(viewLifecycleOwner) { state ->
            when (state) {
                VpnStateService.State.CONNECTED -> {
                    ...
                    animator?.cancel()
                    binding.viewProgress.layoutParams = (binding.viewProgress.layoutParams as FrameLayout.LayoutParams).apply {
    ...
    private fun openServerList() {
        FragmentUtils.getParent(this, MainTabUIDelegate::class.java)?.setCurrentTab(BottomNavBar.TAB_SERVERS)
    }

    private fun updateServerButton(value: Server? = null, forceSave: Boolean = false) {
